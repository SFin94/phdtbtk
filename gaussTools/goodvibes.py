...
import phdtbtk.baseTools.dataframe_funcs as dfuncs
...
def check_thermo_results(mol_data, goodvibes_data, calculation_properties):
    ...
    '''Checks that RRHO thermodynamic quantities are the same across the goodvibes and gaussian thermochemistry results. If not then prints a warning.
    ...
    goodvibes_data[:] *= 2625.5
    mol_data['E SCF (h)'] *= 2625.5
    ...
    comp = dfuncs.data_comp(mol_data, goodvibes_data, df_cols_1=mol_cols, df_cols_2=goodvibes_cols, tol=1e-2)
    if all(comp):
        return
    
    # Try comaprison with scaled gaussian data
    elif ('scale_factor' in calculation_properties):
        scale_factor = calculation_properties['scale_factor']
        mol_data['ZPE_scaled'] = scale_factor*mol_data['ZPE']
        mol_data['H_scaled'] = mol_data['H'] - mol_data['ZPE'] + mol_data['ZPE_scaled']
        mol_cols = ['ZPE_scaled', 'E SCF (h)', 'H_scaled']
        comp = dfuncs.data_comp(mol_data, goodvibes_data, df_cols_1=mol_cols, df_cols_2=goodvibes_cols[:-1], tol=1e-2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("goodvibes_file", type=str, help="Goodvibes results file")
    parser.add_argument("molecule_data", type=str, help="Existing datafile (.csv) of molecule data for goodvibes results to be appended too")
    parser.add_argument("-n", "--new", dest="new_file", type=bool, default=False, help="Flag of whether a new file should be created with the results [default] or append to the old file")
    parser.add_argument("-s", "--save", dest="save", type=str, help="Name of file to save new results too (minus .csv extension)")
    args = parser.parse_args()
    save = args.save

    # Create initial dataframes for existing molecule results and goodvibes results
    mol_data_initial = pd.read_csv(args.molecule_data)
    gv_data, gv_calc_properties = parse_goodvibes(args.goodvibes_file)
    ...
    # Save file as new file or rewrite old one
    if args.new_file == False:
        if save == None:
            save = args.molecule_data.split('.')[0] + '_goodvibes'
        mol_data_full.to_csv(save + '.csv')    
    else:
        mol_data_full.to_csv(args.molecule_data)
